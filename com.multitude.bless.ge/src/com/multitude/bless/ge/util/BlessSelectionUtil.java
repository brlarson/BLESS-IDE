 ...
 *
 */
public final class BlessSelectionUtil {
	/**
	 * Private constructor to prevent instantiation.
	 */
	private BlessSelectionUtil() {
	}
	 ...
	 * @return an optional that contains a DefaultAnnexSubclause for a behavior annex subclause if one is available for the selection. 
	 * Otherwise, an empty optional is returned.
	 */
	public static Optional<DefaultAnnexSubclause> getBLESSAnnexSubclause(final ISelection selection,
	...
	private static Optional<DefaultAnnexSubclause> findDiagramContextForSelectedObject(final Object element) {
		if (element instanceof BLESSSubclause || element instanceof BehaviorState
				|| element instanceof BehaviorTransition || element instanceof VariableDeclaration) {
			return findDiagramContextForSelectedObject(((Element) element).getOwner());
		}

		if (element instanceof DefaultAnnexSubclause
				&& BlessReferenceUtil.ANNEX_NAME.equalsIgnoreCase(((NamedElement) element).getName())) {
