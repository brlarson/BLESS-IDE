 ...
 */
public class InternalConditionHandler implements BusinessObjectHandler {
	private final Graphic labelGraphic = LabelBuilder.create().build();

	@Override
	public boolean isApplicable(final IsApplicableContext ctx) {
		return ctx.getBusinessObject(InternalCondition.class).isPresent();
	}

	@Override
	public CanonicalBusinessObjectReference getCanonicalReference(final ReferenceContext ctx) {
		final InternalCondition bc = ctx.getBusinessObject(InternalCondition.class)
				.orElseThrow();
		return new CanonicalBusinessObjectReference(BlessReferenceUtil.INTERNAL_CONDITION,
				ctx.getReferenceBuilder().getCanonicalReference(bc.getOwner()).encode());
	}

	@Override
	public RelativeBusinessObjectReference getRelativeReference(final ReferenceContext ctx) {
		return new RelativeBusinessObjectReference(BlessReferenceUtil.INTERNAL_CONDITION);
	}
	...
	public String getNameForDiagram(final GetNameForDiagramContext ctx) {
		return ctx.getBusinessObjectContext()
				.getBusinessObject(InternalCondition.class)
				.map(internal_condition -> {
					final BehaviorTransition bt = (BehaviorTransition) internal_condition.getOwner();
