...
public class DispatchConditionHandler implements BusinessObjectHandler {
	...
	public CanonicalBusinessObjectReference getCanonicalReference(final ReferenceContext ctx) {
		final DispatchCondition bc = ctx.getBusinessObject(DispatchCondition.class)
				.orElseThrow();
		return new CanonicalBusinessObjectReference(BlessReferenceUtil.DISPATCH_CONDITION,
				ctx.getReferenceBuilder().getCanonicalReference(bc.getOwner()).encode());
	}

	@Override
	public RelativeBusinessObjectReference getRelativeReference(final ReferenceContext ctx) {
		return new RelativeBusinessObjectReference(BlessReferenceUtil.DISPATCH_CONDITION);
	}
	...
	public String getNameForDiagram(final GetNameForDiagramContext ctx) {
		return ctx.getBusinessObjectContext()
				...
				.map(dispatch_condition -> {
					final BehaviorTransition bt = (BehaviorTransition) dispatch_condition.getOwner();
