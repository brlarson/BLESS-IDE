 ...
 */
public class ModeConditionHandler implements BusinessObjectHandler {
	private final Graphic labelGraphic = LabelBuilder.create().build();

	@Override
	public boolean isApplicable(final IsApplicableContext ctx) {
		return ctx.getBusinessObject(ModeCondition.class).isPresent();
	}

	@Override
	public CanonicalBusinessObjectReference getCanonicalReference(final ReferenceContext ctx) {
		final ModeCondition bc = ctx.getBusinessObject(ModeCondition.class)
				.orElseThrow();
		return new CanonicalBusinessObjectReference(BlessReferenceUtil.MODE_CONDITION,
				ctx.getReferenceBuilder().getCanonicalReference(bc.getOwner()).encode());
	}

	@Override
	public RelativeBusinessObjectReference getRelativeReference(final ReferenceContext ctx) {
		return new RelativeBusinessObjectReference(BlessReferenceUtil.MODE_CONDITION);
	}
	...
	public String getNameForDiagram(final GetNameForDiagramContext ctx) {
		return ctx.getBusinessObjectContext()
				.getBusinessObject(ModeCondition.class)
				.map(mode_condition -> {
					final BehaviorTransition bt = (BehaviorTransition) mode_condition.getOwner();
